# ============================================================
# CELL 7d — RoBERTa vs Naïve Bayes, LR, SVM
# ============================================================
...
os.makedirs("outputs/comparison_roberta", exist_ok=True)
...
# ── Step 3: Load RoBERTa results ─────────────────────────────
print("\nLoading RoBERTa saved results...")
...
print(f"✓ RoBERTa F1={rob_metrics['F1 Score']} | "
      f"Accuracy={rob_metrics['Accuracy']}")

# ── Step 4: RoBERTa probabilities ────────────────────────────
print("\nReloading RoBERTa model for probability scores...")
...
with torch.no_grad():
    for batch in test_dl:
        ...
        with autocast():
            logits = rob_model(input_ids=ids, attention_mask=mask).logits
        rob_probs_list.extend(
            torch.softmax(logits, dim=1).cpu().numpy())
rob_probs = np.array(rob_probs_list)
del rob_model, test_dl, rob_tokenizer, rob_probs_list
free_memory()
print("✓ RoBERTa probabilities computed")

# ── Consolidate ───────────────────────────────────────────────
MODEL_NAMES = ["RoBERTa", "Naïve Bayes", "Logistic Regression", "SVM"]
COLORS_4    = ["#378ADD", "#1D9E75", "#D85A30", "#7F77DD"]
all_preds   = [rob_preds,  nb_preds,  lr_preds,  svm_preds]
all_probs   = [rob_probs,  nb_probs,  lr_probs,  svm_probs]
all_labels  = [rob_labels, y_test,    y_test,    y_test   ]
y_bin_test  = label_binarize(y_test,    classes=[0,1,2])
y_bin_rob   = label_binarize(rob_labels, classes=[0,1,2])
y_bins      = [y_bin_rob, y_bin_test, y_bin_test, y_bin_test]
...
metrics_list = [
    get_metrics("RoBERTa",            rob_labels, rob_preds ),
...
metrics_df.to_csv("outputs/comparison_roberta/roberta_vs_ml_metrics.csv")
print("✓ roberta_vs_ml_metrics.csv saved")
...
bc = ["#378ADD","#1D9E75","#D85A30","#7F77DD"]
...
ax.set_title("RoBERTa vs ML Models — Overall Metrics",
             fontsize=13, fontweight="bold")
ax.legend(loc="lower right")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_overall_metrics.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_overall_metrics.png")

# ── Plot 2: F1 ranking horizontal bar ────────────────────────
sorted_df = metrics_df.sort_values("F1 Score", ascending=True)
fig, ax   = plt.subplots(figsize=(9, 4))
cs = ["#378ADD" if n=="RoBERTa" else "#888780" for n in sorted_df.index]
...
ax.set_xlim(0, 1.15)
ax.set_title("F1 Score Ranking — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
ax.set_xlabel("Weighted F1 Score")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_f1_ranking.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_f1_ranking.png")
...
ax.set_title("Per-class F1 — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
ax.legend()
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_per_class_f1.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_per_class_f1.png")
...
plt.suptitle("Confusion Matrices — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_confusion_matrices.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_confusion_matrices.png")
...
plt.suptitle("ROC Curves — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_roc_curves.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_roc_curves.png")
...
plt.suptitle("Precision-Recall Curves — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_pr_curves.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_pr_curves.png")

# ── Plot 7: Metrics heatmap ───────────────────────────────────
fig, ax = plt.subplots(figsize=(9, 4))
hm      = metrics_df[metric_cols].values.astype(float)
im      = ax.imshow(hm, cmap="YlGn", aspect="auto", vmin=0, vmax=1)
...
ax.set_title("Metrics Heatmap — RoBERTa vs ML Models",
             fontsize=13, fontweight="bold")
plt.tight_layout()
plt.savefig("outputs/comparison_roberta/rob_vs_ml_metrics_heatmap.png",
            dpi=130, bbox_inches="tight")
plt.close()
print("✓ rob_vs_ml_metrics_heatmap.png")

# ── Cleanup ───────────────────────────────────────────────────
del nb, lr, svm, svm_base, X_tr_tfidf, X_te_tfidf, tfidf
del rob_probs, nb_probs, lr_probs, svm_probs
del rob_preds, nb_preds, lr_preds, svm_preds
gc.collect()
print(f"\nGPU: {torch.cuda.memory_allocated()/1e9:.2f} GB")
print("\n✓ Cell 7d complete — outputs/comparison_roberta/")
print("  Run Cell 7e next")
